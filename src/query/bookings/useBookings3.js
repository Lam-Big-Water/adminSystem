...
import { getBookings } from "../../server/apiBookings3";
...
export function useBookings() {
  ...
  const {isPending, data} = useQuery({
    queryKey: ["bookings3", filter, page, search],
  ...
  if (page < pageCount)
    queryClient.prefetchQuery({
      queryKey: ["bookings3", filter, page + 1, search],
      queryFn: () => getBookings({filter, page: page + 1, search})
    })
